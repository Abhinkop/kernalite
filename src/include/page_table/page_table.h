...
typedef struct {
	union {
		...
		struct {
			 ...
			 * Must be 1 (at levels 0-2) to indicate this is a table
			 * descriptor pointing to the next level page table. If
			 * 0, this is a block descriptor (maps a large region
			 * directly).
			 */
			uint64_t table : 1;
			...
			/**
			 * @brief [7:2] Ignored.
			 * These bits are ignored by hardware and available for
			 * software use.
			 */
			 ...
			 * - TCR_ELx.DS=1 (52-bit PA, FEAT_LPA) with 4KB or 16KB
			 * granule: NLTA[51:50]. These bits extend the next
			 * level table address from 48 to 52 bits. Not used with
			 * 64KB granule as it uses a different encoding (see
			 * [47:12]).
			 */
			 ...
			 * - Hardware managed AF enabled (FEAT_HAFDBS): set by
			 * hardware on first access to this table descriptor
			 * itself.
			 *
			 * Note: This is distinct from the AF in page/block
			 * descriptors (L3), which tracks access to the mapped
			 * page. The AF here tracks access to the table
			 * descriptor itself. The OS can clear AF and use it to
			 * implement LRU-based TLB eviction — if AF is still 0
			 * after some period, the table has not been accessed
			 * and is a candidate for reclamation.
			 */
			 ...
			 * Physical address of the next level page table (page
			 * aligned). Exact bits used depend on granule size:
			 ...
			 * - DS=1 with 4KB or 16KB granule (FEAT_LPA):
			 * NLTA[49:48]. Extends the next level table address
			 * beyond 48 bits.
			 */
			 ...
			 * - PnCH=0: IGNORED.
			 * - PnCH=1: marks all memory reachable through this
			 * entry as Stage 1 Protected. Used for
			 * security-sensitive regions such as cryptographic key
			 * storage. Affects cacheability and access behavior of
			 * subordinate entries.
			 */
			...
			/**
			 * @brief [59] Privileged Execute-Never Table bit
			 * (PXNTable). Hierarchical control — applies to all
			 * pages reachable below.
			 * - Two privilege levels: PXNTable.
			 *   If 1, execution at EL1 (privileged) is not
			 * permitted for any page reachable through this table
			 * entry.
			 * - Single privilege level or NV/NV1={1,1}: RES0, must
			 * be zero.
			 */
			uint64_t pxn_table : 1;

			/**
			 * @brief [60] Execute-Never Table bit (XNTable /
			 * UXNTable / PXNTable). Hierarchical control — applies
			 * to all pages reachable below. Interpretation depends
			 * on translation regime:
			 * - Single privilege level:        XNTable.
			 *   If 1, execution is not permitted at any level.
			 * - Two privilege levels (EL0+EL1): UXNTable.
			 *   If 1, execution at EL0 (unprivileged) is not
			 * permitted.
			...
			/**
			 * @brief [62:61] Access Permission Table bits
			 * (APTable[1:0]). Hierarchical control — restricts
			 * access permissions for all pages reachable through
			 * this table entry. Only applies when hierarchical
			 ...
			 *   If 1, all memory reachable through this entry is
			 * Non-Secure. If 0, memory is Secure.
			 * - Non-Secure state: RES0, must be zero.
			 *   (No Secure/Non-Secure distinction exists outside
			 * Secure state.)
			 */
			uint64_t ns_table : 1;
